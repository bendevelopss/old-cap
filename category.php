...
else if(isset($_POST['btnAdd']) && !empty($cname) )
...
$cname1= mysql_real_escape_string($cname);


$status="Active";


mysql_query("insert into category (category_name,  status) values('".$cname1."','".$status."')");
  
 echo '<script type="text/javascript">alert("Category has been added")</script>'; 
 echo "<meta http-equiv='refresh' content='0'>";
}
...
if(!preg_match("/^[a-zA-Z,-,', .,-]*$/",$cname)) 
 ...
 echo '<script type="text/javascript">alert("Invalid Category")</script>'; 
...
?>
    ...
    <input class="w3-input w3-border" type="hidden" name="Category_no" value="<?php echo $hell; ?>" style="width:35%;margin-left:17px; height:30px;" readonly>
...
<div class="w3-row-padding">
  <div class="w3-half">
    <label style="margin-left:-180px;">Category Name:</label>
    <strong><span id='messages' style="margin-left:-5px; color: red;"></span></strong>
    
    <input class="w3-input w3-border" type="text" placeholder="Category Name" name="txtname" id="categoryname" style="height:33px;">
...
mysql_query("update category set status='".$status."' 
 ...
 echo '<script type="text/javascript">alert("Category has been deleted")</script>'; 
 echo "<meta http-equiv='refresh' content='0'>";
  ...
  <table class="dt-responsive table-bordered w3-table w3-bordered w3-striped w3-border w3-hoverable" id="tableko1" name="tableko1" style="font-size: 0.9em;">
    ...
    while($row = $result->fetch_assoc())
      { 
        ...
        $category_name=$row['category_name'];
      echo'<tr>';
       echo'<td>'.$category_no.'</td>';
        echo'<td>'.$category_name.'</td>';
        ?>
        <td style="text-align:center"><button type="submit" name="btnRemove" value="<?php echo''.$category_no.''; ?>" class="btn btn-primary btn btn-danger fa fa-close btn-xs"  onclick="return confirm('Are you sure?');"></button>
        <?php
       echo'<button type="button" name="btnEdit" id="bntEdit" value="'.$category_no.'" data-toggle="modal" data-target="#myModal" class="btn btn-primary glyphicon glyphicon-pencil btn-xs center" onclick="get_id(this)" ></button>
         </td>';
        }
      echo'</tr>';
     echo' 
    ...
    <div class="modal-dialog">
        ...
        <form method ="post" role="form" id="editForm">
            ...
            <div class="modal-body">
              ...
              <input class="form-control" name="category_no1" id="category_no1" value="" readonly/>
...
<!--modal get the value-->
 ...
 $category_no1 = $modal.find('#category_no1');
            $category_no1.val(document.getElementById("tableko1").rows[($(o).parent().parent().index())+1].cells[0].innerHTML);
...
function ClearFields() {
   ...
   var category = $("#categoryname").val();
      ...
      document.getElementById("categoryname").value = "";
       $("#categoryname").removeClass('w3-border-green');
 $("#categoryname").removeClass('w3-border-green w3-pale-green w3-leftbar w3-rightbar w3-border-green');
...
$(document).ready(function () {
   $("#categoryname, #dup").keyup(checkPasswordMatch);
});
...
$("#messages").html("required").css('color','red');
      $("#categoryname").removeClass('w3-border-red');
      $("#categoryname").removeClass('w3-border-green');
      $("#categoryname").removeClass('w3-border-red w3-pale-red w3-leftbar w3-rightbar w3-border-red');
      $("#categoryname").removeClass('w3-border-green w3-pale-green w3-leftbar w3-rightbar w3-border-green');
      $("#messages").removeClass('fa fa-thumbs-o-down');
      $("#messages").removeClass('fa fa-thumbs-o-up');
 ...
 else
    {
        ...
        $("#categoryname").removeClass('w3-border-red w3-pale-red w3-leftbar w3-rightbar w3-border-red').addClass('w3-border-green w3-pale-green w3-leftbar w3-rightbar w3-border-green');
    }
...
var category, text;
 ...
 category = document.getElementById("categoryname").value;
  ...
  if (category=='') 
    {
        alert("Category Name is a required filled");
        return false;
    } 
    else
