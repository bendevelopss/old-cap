...
?>
<div class="w3-half">
<label style="margin-left:-350px;">Delivery ID:</label>
...
<div class="container" style="width:98%; margin-left: 10px;">
  ...
  <table class="table-bordered w3-table w3-bordered w3-striped w3-border w3-hoverable" id="tableko" name="tableko" >
    ...
    while($row = $result->fetch_assoc())
      { 
      echo'<tr>';
      echo'<td>'.ucfirst($row['po']).'</td>';
        echo'<td>'.ucfirst($row['supp']).' </td>';
        echo'<td>'.ucfirst($row['date']).'</td>';
        echo'<td>'.ucfirst($row['ordered']).'</td>';
        echo'<td style="text-align:center;">';
        echo'<button type="button" name="btnNext" onclick="return myFunctions();" class="w3-btn w3-green w3-border w3-round-large"><a href="delivery2.php?id='.$hell.'&scname='.$row['po'].'&prepared='.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'" style="color:White; ">Transact</a> </a></button> ';
        }
      echo'</tr>';
     echo' 
    </tbody>
