...
<ul class="w3-navbar w3-card-4 w3-camo-dark-green w3-large">
  <li><a href="#"><i class="fa fa-home" style="height: 22px; text-align: center;">  Fareal Builders Inc.</i></a></li>
    ...
    <li class="w3-right w3-dropdown-hover w3-hover-orange">
    <a class="w3-black w3-hover-black" href="#"><i class="fa fa-user"></i></a>
    <div class="w3-dropdown-content w3-white w3-card-4" style="right:0">';?>  
    <h4><?php echo ''.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'.'; ?></h4>  
      <?php echo'<h5><a href="customer.php">User Settings</a></h5>
      <h5><a href="?logout=true">Logout</a></h5>
    </a></li>
    <li class="w3-right" style="margin-top:8px;"><span id="time" style="margin-left: 800px;"></span></li>
</ul>';
...
mysql_query("insert into payment (customer,topay, amount, bankname, chequeno, chequedate, type, status ) 
  values('". $customer."','".$topay."', '".$amount."','".$bankname."'  ,'".$chequeno."', '".$chequedate."','".$radio."','".$status."')");
  
 echo '<script type="text/javascript">alert("'.$topay.'")</script>'; 
...
<label style="margin-left:-160px;"> Amount</label>
 <strong><span id='messageuser' style="margin-left:10px;"></span></strong>
    <input class="w3-input w3-border" type="text" placeholder="Total Amount to Pay" style="height:33px;" id="amount" name="amount">
  </div>
...
<button type="button" name="btnprint" id="btnprint" onclick="print(this)" value ="'.$payment_no.'" class="w3-btn">Print</button>
 ...
 <script>
    ...
    window.open("pdf/tutorial/tutopay.php?payment_no="+getid+"& customer=<?php echo ''.$customer.'';?>&amountopay=<?php echo ''.$topay.'';?>&amount=<?php echo ''.$amount.'';?>");
    }
    </script>
