...
<header class="w3-container w3-camo-dark-green w3-round">
  <h1>Transaction: Purchase Order</h1>
</header>
<!--card Header-->
...
?>
<div class="w3-half">
<label style="margin-left:-350px;">Purchase Order ID:</label>
    <input class="w3-input w3-border" type="text" id="quote_no" name="quote_no" value="<?php echo $hell; ?>" style="width:30%;margin-left:100px; height:30px;" readonly>
    </div>
<div class="w3-half">
  ...
  <table class="table-bordered w3-table w3-bordered w3-striped w3-border w3-hoverable" id="tableko" name="tableko" >
    ...
    while($row = $result->fetch_assoc())
      ...
      echo'<td>'.ucfirst($row['supp_name']).'</td>';
        ...
        echo'<button type="button" name="btnNext" onclick="return myFunctions();" class="w3-btn w3-green w3-border w3-round-large"><a href="purchaseorder2.php?po_no='.$hell.'&scname='.$row['supplier_no'].'&ordered='.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'" style="color:White; ">Order</a> </a></button> ';
        }
      echo'</tr>';
     echo' 
    </tbody>
...
<div class="w3-row-padding">
  <div class="w3-third">
    <label style="margin-left:-155px;">Ordered by:</label>
...
var category, text;
 ...
 {
         window.location.href="quotation2.php?id="+id+"&customer=<?php echo''.$customer.'' ?>&scname="+category+"&prepared=<?php echo ''.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'.'; ?>";
