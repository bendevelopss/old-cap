...
<label style="margin-left:-350px;">Quotation ID:</label>
    <input class="w3-input w3-border" type="text" id ="quote_no" name="quote_no" value="<?php echo $hell; ?>" style="width:30%;margin-left:100px; height:30px; text-align: center;" readonly>
    </div>
<div class="w3-half">
  ...
  <table class="table-bordered w3-table w3-bordered w3-striped w3-border w3-hoverable" id="tableko" name="tableko" >
    ...
    while($row = $result->fetch_assoc())
      ...
      echo'<td>'.ucfirst($row['quote_no']).'</td>';
        ...
        echo'<button type="button" name="btnNext" onclick="return myFunctions();" class="w3-btn w3-green w3-border w3-round-large"><a href="quotation2.php?id='.$hell.'&scname='.$row['quote_no'].'&prepared='.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'" style="color:White; ">Accept</a> </a></button> 

        <button type="button" name="btnDec" value="'.$row['quote_no'].'" onclick="return done();" class="w3-btn w3-red w3-border w3-border-red w3-round-large">Decline</a></button></td>';
       }
         else if($row['status']=='active')
        {

        echo'<button type="submit" name="btnNext" class="w3-btn w3-green w3-border w3-round-large"><a href="pdf/tutorial/tuto9.php?quote_no='.$row['quote_no'].'&id='.$row['quote_no'].'&prepared='.$row['prepared_by'].'" target="_blank" style="color:White; ">Print</a> </a></button> 

        <button type="submit" value="'.$row['quote_no'].'" class="w3-btn w3-blue w3-border w3-border-blue w3-round-large"><a href="view.php?project='.ucfirst($row['project']).'&id='.$row['quote_no'].'" style="color:white; ">View</a></button></td>';
       }
        }
      echo'</tr>';
     echo' 
...
<div class="w3-row-padding">
  <div class="w3-third">
    <label style="margin-left:-155px;">Prepared by:</label>
    <input class="w3-input w3-border" type="text" placeholder="Prepared by" style="height:33px; margin-left:80px;  width: 85%;" id="prepared" name="prepared" value="<?php echo ''.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'.'; ?>" readonly>
  </div>
</div>
<br>
<!--Left Body-->
